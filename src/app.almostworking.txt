...
const GoogleSignInButton = ({ text = "Continue with Google" }) => {
  ...
  return (
    <button
      ...
      className="w-full bg-white hover:bg-gray-50 text-gray-800 font-semibold py-3 px-4 rounded-xl border-2 border-gray-200 transition duration-200 flex items-center justify-center gap-3"
    >
...
const SignupPage = ({ onSwitchToLogin, onSignupSuccess }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Check BOTH success and status fields (backend might use either)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isSuccess) {
        ...
        login(userData);
        console.log("User state should now be updated");
      } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isSuccess) {
        ...
        const errorMsg = result?.error || result?.message || "Invalid OTP. Please try again.";
...
export default App;import React, { useState, createContext, useContext, useEffect } from "react";
import {
  Wallet,
  TrendingUp,
  PieChart,
